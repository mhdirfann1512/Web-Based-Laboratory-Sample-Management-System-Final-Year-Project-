...
console.log("🔧 Completely fixing app.js syntax errors and export routes...")

const appJsPath = path.join(process.cwd(), "app.js")
let appJsContent = fs.readFileSync(appJsPath, "utf8")

// Remove all the broken export routes and duplicate code
const cleanupPatterns = [
  // Remove broken export routes that start with "// Fetch samples data"
  /\/\/ Fetch samples data[\s\S]*?(?=app\.use\(|const PORT|process\.on)/g,
  // Remove duplicate export routes
  /app\.get$$'\/api\/admin\/export\/samples\/:\w+',[\s\S]*?}$$;/g,
  // Remove any standalone await statements
  /^\s*const \[.*?\] = await db\.promise$$$$\.query$$[\s\S]*?$$;?\s*$/gm,
  // Remove malformed route definitions
  /^\s*if \(format === ['"]csv['"][\s\S]*?}\s*$/gm,
]

cleanupPatterns.forEach((pattern) => {
  appJsContent = appJsContent.replace(pattern, "")
})

// Add the working export routes before the error handlers
const workingExportRoutes = `
...
// Insert the working export routes before the error handlers
const errorHandlerIndex = appJsContent.indexOf("app.use((err, req, res, next)")
if (errorHandlerIndex !== -1) {
  appJsContent =
    appJsContent.slice(0, errorHandlerIndex) + workingExportRoutes + "\n\n" + appJsContent.slice(errorHandlerIndex)
} else {
  // If no error handler found, add before the PORT definition
  const portIndex = appJsContent.indexOf("const PORT")
  if (portIndex !== -1) {
    appJsContent = appJsContent.slice(0, portIndex) + workingExportRoutes + "\n\n" + appJsContent.slice(portIndex)
  }
}
...
console.log("✅ Added working export routes")
console.log("✅ Removed duplicate and broken code")
console.log("📋 Your server should now start without errors!")
console.log("📋 Export functionality will work: CSV, JSON, and HTML (printable as PDF)")
