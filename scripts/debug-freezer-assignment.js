// Debug script to test freezer assignment logic
require("dotenv").config()
const mysql = require("mysql2/promise")

async function debugFreezerAssignment() {
  try {
    ...
    for (const storage of storageLocations) {
      ...
      // Simulate the freezer assignment logic from app.js
      let freezerQuery = "SELECT * FROM freezer WHERE status = 'active' OR status IS NULL"
      const freezerParams = []
      ...
      if (matchedFreezers.length > 0) {
        ...
        // Try fallback
        const [fallbackFreezers] = await connection.execute(
          "SELECT * FROM freezer WHERE status = 'active' OR status IS NULL ORDER BY freezer_id LIMIT 1",
        )
