...
console.log("🔧 Replacing export routes with built-in Node.js functionality...")

const appJsPath = path.join(process.cwd(), "app.js")
let appJsContent = fs.readFileSync(appJsPath, "utf8")

// Remove the problematic export routes that use external packages
const exportRoutesRegex = /\/\/ Export API Routes[\s\S]*?(?=\/\/ |app\.use\(|const PORT|process\.on)/
appJsContent = appJsContent.replace(exportRoutesRegex, "")

// Add simple export routes using only built-in Node.js
const simpleExportRoutes = `
...
// Insert the new export routes before the error handlers
const errorHandlerIndex = appJsContent.indexOf("app.use((err, req, res, next)")
if (errorHandlerIndex !== -1) {
  appJsContent =
    appJsContent.slice(0, errorHandlerIndex) + simpleExportRoutes + "\n\n" + appJsContent.slice(errorHandlerIndex)
  fs.writeFileSync(appJsPath, appJsContent)
  console.log("✅ Replaced export routes with built-in Node.js functionality")
} else {
  console.log("❌ Could not find error handler in app.js")
}

console.log("✅ Export functionality now works without external packages!")
console.log("📋 Restart your server - exports will work immediately!")
